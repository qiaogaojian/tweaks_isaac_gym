...
def _create_envs(gym, sim):
    ...
    p_gains = torch.zeros(num_envs, num_actions, dtype=torch.float, device=device, requires_grad=False)
    d_gains = torch.zeros(num_envs, num_actions, dtype=torch.float, device=device, requires_grad=False)
...
def step(actions):
    ...
    for _ in range(cfg.control.decimation):
        torques = _compute_torques(actions).view(torques.shape)
...
def _compute_torques(actions):
    ...
    """
    # pd controller
    target_q = actions * cfg.control.action_scale + default_dof_pos - dof_pos
    p_gains = p_gains
    d_gains = d_gains
    # target_q[i] = (action[i] * cfg.action_scales + cfg.default_dof_pos[i]) * cfg.motor_direction[i]
    torques = p_gains * (target_q) - d_gains * dof_vel
